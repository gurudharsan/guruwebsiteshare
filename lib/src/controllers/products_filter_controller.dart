...
class ProductsFilterController extends GetxController {
  ProductsFilterController(
      {this.categoryId = 0, this.title = "", this.subtitle = ""});
  ...
  RxList<CategoryProductData> reagentsFilteredData =
      <CategoryProductData>[].obs;
  RxList<CategoryProductData> reagentsSearchFilteredData =
      <CategoryProductData>[].obs;
  ...
  getFilteredProducts({String query = ""}) async {
    ...
    await Repository()
        .getFilteredCategoryProducts(
            ...
            search: query,
            filtersCallback:
                mainFilterType.value == 2 ? getFilterDataFromApi : null)
        .then((value) {
  ...
  combinationFilterAfterApiCall(List<CategoryProductData> products) {
    ...
    reagentsFilteredData.value.sort((a, b) {
  ...
  getFilterDataFromApi(Map<String, dynamic> filterData) {
    if (brandFilters.isEmpty &&
        parameterFilters.isEmpty &&
        productFilters.isEmpty) {
      brandFilters.value = filterData['brands']!;
      parameterFilters.value = filterData['tags']!;
      try {
        productFilters.value = (filterData['products'] as Map<String, String>)
            .keys
            .toSet()
            .toList();
      } catch (e) {
    ...
    for (var element in instruments) {
      if (immunologySubFilterList.contains(element.ordering) &&
          selectedImmunologyInstrumentFilters
              .contains(element.instrumentType)) {
        filteredData.add(element);
      }
    }
  }
  ...
  getImmunologyInstrumentsFilter() {
    filteredData.value = [];
    for (var element in instruments) {
      if (element.instrumentType!.isNotEmpty &&
          !immunologyInstrumentFilters.contains(element.instrumentType)) {
        immunologyInstrumentFilters.add(element.instrumentType!);
      }
    }
    selectedImmunologyInstrumentFilters
        .addAll(immunologyInstrumentFilters.toSet());
  }
  ...
  filterInstruments() {
    ...
    filteredData.value = instruments
        .where((v) => selectedInstrumentFilters.contains(v.instrumentType))
        .toList();
  }
  ...
  getFilterItems() {
    ...
    if (mainFilterType.value == 1) {
      for (var value in instruments) {
        if (value.ordering == 1) {
          if (value.instrumentType!.isNotEmpty &&
              !instrumentFilters.contains(value.instrumentType)) {
      ...
      for (var value in reagents) {
        if (value.ordering == 2) {
          ...
          if (value.tags!.isNotEmpty &&
              !parameterFilters.contains(value.tags)) {
  ...
  searchReagents(String search) {
    ...
    for (var element in reagentsFilteredData) {
      ...
      if (element.shortDescription != null &&
          element.shortDescription!.toLowerCase().contains(query)) {
        if (!reagentsSearchFilteredData.contains(element)) {
          reagentsSearchFilteredData.add(element);
        }
      }
