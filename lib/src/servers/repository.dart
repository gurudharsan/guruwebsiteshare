...
import 'package:cpcdiagnostics_ecommerce/src/models/best_selling_product_model.dart'
    as best_sell;
import 'package:cpcdiagnostics_ecommerce/src/models/campaign_details_model.dart';
...
import 'package:cpcdiagnostics_ecommerce/src/models/coupon_list_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/models/edit_view_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/models/favorite_product_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/models/my_download_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/models/my_reward_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/models/my_wallet_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/models/offer_ending_product_model.dart'
    as offer_ending;
import 'package:cpcdiagnostics_ecommerce/src/models/home_data_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/models/order_list_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/models/product_by_brand_model.dart'
    as brand;
import 'package:cpcdiagnostics_ecommerce/src/models/product_by_campaign_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/models/product_by_category_model.dart'
    as product;
import 'package:cpcdiagnostics_ecommerce/src/models/product_by_shop_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/models/product_details_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/models/profile_data_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/models/search_product_model.dart';
...
import 'package:cpcdiagnostics_ecommerce/src/models/recent_viewed_product_model.dart'
    as recent_product;
import 'package:cpcdiagnostics_ecommerce/src/models/visit_shop_model.dart';
import 'package:cpcdiagnostics_ecommerce/src/screen/auth_screen/otp_screen.dart';
import 'package:cpcdiagnostics_ecommerce/src/servers/network_service.dart';
...
class Repository {
  ...
  Future addToCartWithTrxId(
    ...
    printLog(LocalDataHelper().getUserToken());
    ...
    if (response.statusCode == 200) {
      ...
      showShortToast("Added to Inquiry List");
    } else {
      showErrorToast(data['message']);
    }
  }
  ...
  Future<AddToCartListModel> getCartItemList() async {
    ...
    var headers = {"apiKey": Config.apiKey};
    Uri url;
    print("calling cart with trxid-->${LocalDataHelper().getCartTrxId()}");
    if (LocalDataHelper().getUserToken() == null) {
      print("calling cart without user token");
      url = Uri.parse(
          "${NetworkService.apiUrl}/carts?trx_id=${LocalDataHelper().getCartTrxId()}&$langCurrCode");
    } else {
      print("calling cart with token");
    ...
    addToCartListModel.data!.carts!.forEach((element) {
      element.productName;
    });
      ...
      for (var element in productsData) {
        if ((element['linked_products'] as List).isNotEmpty) {
          for (var filteredReagent in (element['linked_products'] as List)) {
            products.add(product.CategoryProductData.fromJson(filteredReagent));
          }
        }
      }
    }
  ...
  //Product Details
  Future<List<BrochureModel>> getBrochures(int page) async {
    print(page);
